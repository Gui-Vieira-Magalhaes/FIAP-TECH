...
forecast_period = st.slider("Escolha o período de previsão (em dias):", 30, 5844, 90)
...
# Gráfico comparativo LSTM
st.subheader("Previsão LSTM Comparativa")
fig, ax = plt.subplots(figsize=(12, 6))
ax.plot(range(len(y_test_lstm)), scaler.inverse_transform(y_test_lstm.reshape(-1, 1)), label='Valores Reais', color='blue', alpha=0.6)
ax.plot(range(len(y_pred_lstm)), y_pred_lstm, label='Previsões LSTM', color='orange', alpha=0.6)
